...
\begin{center}
    ...
    \begin{tabular}{rl}
        ...
        \textbf{Báo cáo PDF:} & \href{https://github.com/23001853-wq/Employee-Attendance-System-Using-Camera/blob/main/report.pdf}{report.pdf} \\
        \textbf{Slide thuyết trình:} & \href{https://github.com/23001853-wq/Employee-Attendance-System-Using-Camera/blob/main/slides.pdf}{slides.pdf} \\
...
\begin{enumerate}
    \item \textbf{SVM (RBF Kernel):} Sử dụng Grid Search để tìm siêu tham số tối ưu ($C=1.5$, $\gamma=\text{scale}$).
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{ketquatt1.png}
...
\begin{table}[h!]
    ...
    \begin{tabular}{lccc}
        ...
        \hline
        SVM (RBF Kernel)     & 97      & 0.07   & Tốt nhất, chấp nhận được \\
        KNN (k=5)            & 97.5      & 0.04   & Nhanh hơn SVM \\
        Random Forest        & 96      & 0.005   & Nhanh nhất (giám sát) \\
        Cosine Similarity    & 80      & 0.002   & Không giám sát, nhanh \\
        L2 Distance          & 80      & 0.0015   & Nhanh nhất tất cả \\
        \hline
    \end{tabular}
    \caption{So sánh các thuật toán nhận diện khuôn mặt (Test Accuracy)}
...
\begin{table}[h!]
    ...
    \begin{tabular}{lcc}
        ...
        \hline
        SVM (RBF Kernel)  & 89.30\% & ±7.58\% \\
        Random Forest     & 76.80\% & ±4.85\% \\
        KNN (k=5)         & 75.30\% & ±9.84\% \\
        \hline
    \end{tabular}
    \caption{Kết quả Cross-Validation 5-Fold trên toàn bộ dataset (1000 ảnh, 20 nhân viên)}
...
\textbf{Ưu điểm:}
\begin{itemize}
    \item \textbf{Độ chính xác cao nhất:} Đạt 97.5\% trên test set, đồng thời có Cross-Validation accuracy = 89.30\% với std dev ±7.58\%, chứng tỏ mô hình ổn định trên nhiều phân chia dữ liệu khác nhau. Độ chênh lệch giữa CV và Test là 7.6\%, cho thấy test set có phân bố thuận lợi hơn các fold trong CV.
    
    \item \textbf{Khả năng tổng quát hóa tốt:} RBF Kernel giúp SVM xử lý được dữ liệu không tuyến tính trong không gian 1280 chiều. Kết hợp với chuẩn hóa L2, mô hình phân tách các lớp hiệu quả ngay cả khi có thay đổi về ánh sáng và góc chụp.
    
    \item \textbf{Thời gian xử lý chấp nhận được:} 0.07 giây cho 200 ảnh test đủ nhanh cho ứng dụng real-time điểm danh, cân bằng giữa độ chính xác và tốc độ.
    
    \item \textbf{Hoạt động tốt với dữ liệu nhỏ:} Với chỉ 50 ảnh/nhân viên, SVM vẫn học được ranh giới phân lớp rõ ràng mà không bị overfitting nghiêm trọng.
\end{itemize}

\textbf{Nhược điểm:}
\begin{itemize}
    \item \textbf{Thời gian huấn luyện tăng khi scale:} Độ phức tạp $O(N^2)$ đến $O(N^3)$ khiến việc huấn luyện chậm khi số lượng nhân viên và ảnh tăng lên đáng kể.
    
    \item \textbf{Cần điều chỉnh siêu tham số:} Phải tìm $C$ và $\gamma$ tối ưu tránh học quá khớp, tốn thời gian thực nghiệm ban đầu.
\end{itemize}
...
\textbf{Ưu điểm:}
\begin{itemize}
    \item \textbf{Không cần huấn luyện:} Lazy learning giúp triển khai nhanh chóng, phù hợp khi cần thêm nhân viên mới mà không cần huấn luyện lại.
    
    \item \textbf{Đơn giản, dễ hiểu:} Thuật toán trực quan, dễ giải thích cho người dùng không chuyên.
    
    \item \textbf{Tốc độ dự đoán nhanh:} 0.04s cho 200 ảnh test, nhanh hơn SVM gần 2 lần, phù hợp cho real-time.
    
    \item \textbf{Độ chính xác cao:} Đạt 97.5\% với cấu hình k=5.
\end{itemize}

\textbf{Nhược điểm:}
\begin{itemize}
    \item \textbf{Độ tin cậy thấp nhất:} Cross-Validation cho 75.30\% (±9.84\%) - độ lệch chuẩn cao nhất trong các thuật toán. Chênh lệch CV-Test lên đến 20.1\% (test: 97.5\%, CV: 75.30\%), cho thấy KNN rất nhạy cảm với cách chia dữ liệu và không ổn định,dẫn đến rất học vẹt. Khi tăng nhân viên,độ chính xác trung bình càng thấp nặng nề.
    
    \item \textbf{Chậm khi scale lên:} Với tập dữ liệu lớn, phải tính khoảng cách đến tất cả mẫu huấn luyện, làm tốc độ giảm mạnh.
    
    \item \textbf{Nhạy cảm với nhiễu:} Một vài ảnh chất lượng kém có thể ảnh hưởng lớn đến kết quả phân loại do cơ chế "bầu chọn đa số".
    
    \item \textbf{Curse of dimensionality:} Trong không gian 1280 chiều, khái niệm "gần nhất" trở nên mơ hồ, hiệu suất giảm so với không gian chiều thấp.
\end{itemize}
...
\textbf{Ưu điểm:}
\begin{itemize}
    \item \textbf{Kháng nhiễu tốt:} Cross-Validation đạt 76.80\% (±4.85\%) - std dev thấp nhất trong 3 thuật toán, cho thấy mô hình ensemble giảm được overfitting và ổn định nhất qua các fold. Random Forest hưởng lợi từ dataset lớn hơn.
    ...
    \item \textbf{Tốc độ xử lý nhanh :} 0.005s cho 200 ảnh test, nhanh hơn SVM gần 13 lần và KNN 7 lần - lý tưởng cho real-time.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Chênh lệch CV-Test lớn:} Test accuracy cao hơn 10\% CV accuracy, cho thấy có dấu hiệu overfitting nhẹ.
...
\textbf{Ưu điểm:}
\begin{itemize}
    \item \textbf{Tốc độ rất nhanh:} Cosine: 0.002s, L2: 0.0015s cho 200 ảnh test. Nhanh hơn SVM 22-50 lần. Chỉ cần so sánh với 20 vector đại diện (centroids) thay vì hàng trăm mẫu.
    ...
    \item \textbf{Kết quả tương đương:} Cosine 80\% và L2 80\% cho kết quả tương đương do tính chất toán học: minimize L2 distance $\approx$ maximize Cosine similarity khi đã chuẩn hóa L2.
    
    \item \textbf{L2 Distance nhanh nhất:} 0.0015s là nhanh nhất trong tất cả các thuật toán, phù hợp cho ứng dụng cần tốc độ tối đa.
\end{itemize}

\textbf{Nhược điểm:}
\begin{itemize}
    \item \textbf{Độ chính xác thấp nhất:} Cosine 80\%, L2 80\% - kém SVM tới ~17\%. Đây là điểm yếu nghiêm trọng, không phù hợp cho môi trường sản xuất yêu cầu độ tin cậy cao.
...
\subsubsection{Kết luận từ phân tích}

Dựa trên các kết quả và phân tích trên dataset  (1000 ảnh, 20 nhân viên):

\begin{itemize}
    \item \textbf{SVM (RBF Kernel)} là lựa chọn tối ưu nhất cho hệ thống điểm danh thực tế, đạt độ chính xác cao nhất (97\%), độ ổn định tốt (CV: 89.30\%±7.58\%, chênh lệch CV-Test ~7\%) và tốc độ chấp nhận được (0.07s cho 200 ảnh test). SVM scale tốt khi tăng số lượng nhân viên.
    
    \item \textbf{KNN (k=5)} có test accuracy cao nhất (97.5\%) nhưng CV accuracy rất thấp (76.38\%±9.05\%) với chênh lệch lên đến ~19\%. Điều này chứng tỏ KNN "may mắn" trên test set cụ thể này và KHÔNG ĐỦ TIN CẬY để triển khai thực tế.
    
    \item \textbf{Random Forest} phù hợp khi cần tốc độ nhanh nhất (0.004s - nhanh gấp 12.5 lần SVM) và chấp nhận độ chính xác thấp hơn một chút (94.8\%). Ưu điểm là kháng nhiễu tốt và ít cần điều chỉnh tham số.
    
    \item \textbf{Cosine/L2 Distance} KHÔNG phù hợp cho ứng dụng thực tế do độ chính xác quá thấp (~80\%). Chỉ nên dùng cho prototype hoặc demo nhanh, không nên triển khai production.
\end{itemize}

\textbf{Kết luận tạm thời:} Sử dụng \textbf{SVM (RBF Kernel)} cho hệ thống chính thức.
